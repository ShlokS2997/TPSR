...
class NesymresDataset(data.Dataset):
    ...
    except (UnknownSymPyOperator, RecursionError) as e:
        logging.error(f"Error processing equation at index {index}: {str(e)}")
        curr = Equation(code=None, expr=[], coeff_dict=consts, variables=eq.variables, support=eq.support, valid=False)

    except Exception as e:
        logging.error(f"Unexpected error at index {index}: {str(e)}")
        curr = Equation(code=None, expr=[], coeff_dict=consts, variables=eq.variables, support=eq.support, valid=False)
